    ...
    def compute_metrics(self, batch):
        ...
        for b in range(batch_size):
            ...
            if len(predicted_edges) != 0:
                ...
                print(f"    Edge Distance Debug:")
                print(f"      edge_distance shape: {edge_distance.shape}")
                print(f"      edge_distance min/max: {edge_distance.min():.4f} / {edge_distance.max():.4f}")
                print(f"      distance_thresh: {self.distance_thresh}")
                print(f"      matched distances: {edge_distance[predict_indices, label_indices]}")
                print(f"      distances <= thresh: {edge_distance[predict_indices, label_indices] <= self.distance_thresh}")
                ...
                # Debug: Print shapes to trace zero-size array issue
                print(f"    AP Calculator Debug:")
                print(f"      pr_corners shape: {pr_corners.shape}")
                print(f"      gt_corners shape: {gt_corners.shape}")
                print(f"      pr_vertices shape: {pr_vertices.shape}")
                print(f"      gt_vertices shape: {gt_vertices.shape}")
                print(f"      edge_mask: {edge_mask}")
                print(f"      sum(edge_mask): {sum(edge_mask)}")
